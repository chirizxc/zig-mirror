...
dwarf_funcs: std.ArrayList(struct {
    frame_first_node_reloc: NodeReloc.Index,
    debug_info_first_node_reloc: NodeReloc.Index,
...
const NodeReloc = struct {
    ...
    const Size = enum { @"32", @"64" };

    const Type = packed struct(u16) {
        fn simple(target: Target, action: Simple) NodeReloc.Type {
            return .{ .target = target, .action = .{ .simple = action } };
        }
    ...
    fn apply(reloc: *NodeReloc, elf: *Elf) void {
        assert(elf.ehdrType() != .REL);
        assert(reloc.node != .none and reloc.target != .none);
    ...
    fn applyInner(reloc: *const NodeReloc, elf: *Elf) error{ RelocationOverflow, RelocationMisaligned }!void {
        ...
        const target_val: u64 = switch (reloc.type.target) {
            .abs => elf.computeNodeVAddr(reloc.target) +% addend,
        };
        try reloc.type.action.simple.write(target_val, dest_slice, elf.targetEndian());
    }
};
...
pub fn addNodeReloc(
    ...
    elf.addNodeRelocAssumeCapacity(node, offset, target, addend, size) catch |err| switch (err) {
        error.MappedFileIo => return diags.fail("failed to write output file: {t}", .{elf.mf.io_err.?}),
        else => |e| return e,
    };
}
...
fn create(
    ...
    const data: std.elf.DATA = switch (std.lang.Endian.native) {
        .little => .@"2LSB",
        .big => .@"2MSB",
    };
    ...
    switch (class) {
        ...
        inline else => |ct_class| {
            ...
            if (@"type" != .REL) {
                ...
                if (target_endian != std.lang.Endian.native) std.mem.byteSwapAllFields(ElfN.Phdr, ph_phdr);
                ...
                if (maybe_interp) |_| {
                    ...
                    if (target_endian != std.lang.Endian.native) std.mem.byteSwapAllFields(ElfN.Phdr, ph_interp);
                }
                ...
                if (target_endian != std.lang.Endian.native) std.mem.byteSwapAllFields(ElfN.Phdr, ph_rodata);
                ...
                if (target_endian != std.lang.Endian.native) std.mem.byteSwapAllFields(ElfN.Phdr, ph_text);
                ...
                if (target_endian != std.lang.Endian.native) std.mem.byteSwapAllFields(ElfN.Phdr, ph_data);
                ...
                if (have_dynamic) {
                    ...
                    if (target_endian != std.lang.Endian.native) std.mem.byteSwapAllFields(ElfN.Phdr, ph_dynamic);
                }
                ...
                if (target_endian != std.lang.Endian.native) std.mem.byteSwapAllFields(ElfN.Phdr, ph_gnu_stack);
            }
...
fn addNodeRelocAssumeCapacity(
    ...
    assert(node != .none and target != .none);
    const shndx = elf.getNodeShndx(target);
    assert(!shndx.flags(elf).ALLOC); // runtime relocations not needed here so not implemented
    const first_node_reloc = switch (elf.getNode(target)) {
        else => unreachable,
        .value_debug_info => |cpi| &elf.dwarf_values.items[@backingInt(cpi)].debug_info_first_node_reloc,
        .global_debug_info => |gi| &elf.dwarf_globals.items[@backingInt(gi)].debug_info_first_node_reloc,
        .func_frame => |gi| &elf.dwarf_funcs.items[@backingInt(gi)].frame_first_node_reloc,
        .func_debug_info => |gi| &elf.dwarf_funcs.items[@backingInt(gi)].debug_info_first_node_reloc,
        .func_debug_line => |gi| &elf.dwarf_funcs.items[@backingInt(gi)].debug_line_first_node_reloc,
    };
    const next = first_node_reloc.*;
    ...
    switch (elf.ehdrType()) {
        .REL => {
            ...
            const rela_index = rela_shndx.relaAddOneAssumeCapacity(elf, .{
                .type = switch (elf.ehdrMachine()) {
                    .AARCH64 => .{ .AARCH64 = switch (size) {
                        .@"32" => .ABS32,
                        .@"64" => .ABS64,
                    } },
                    .LOONGARCH => .{ .LARCH = switch (size) {
                        .@"32" => .@"32",
                        .@"64" => .@"64",
                    } },
                    .PPC64 => .{ .PPC64 = switch (size) {
                        .@"32" => .ADDR32,
                        .@"64" => .ADDR64,
                    } },
                    .RISCV => .{ .RISCV = switch (size) {
                        .@"32" => .@"32",
                        .@"64" => .@"64",
                    } },
                    .SPARCV9 => .{ .SPARC = switch (size) {
                        .@"32" => .UA32,
                        .@"64" => .UA64,
                    } },
                    .X86_64 => .{ .SPARC = switch (size) {
                        .@"32" => .@"32",
                        .@"64" => .@"64",
                    } },
                },
        ...
        .DYN, .EXEC => {
            elf.node_relocs.appendAssumeCapacity(.{
                ...
                .target = target,
                .addend = addend,
                .type = .simple(.abs, .{ .dest = switch (size) {
                    .@"32" => .@"32",
                    .@"64" => .@"64",
...
fn flushMoved(elf: *Elf, ni: MappedFile.Node.Index) std.mem.Allocator.Error!void {
    ...
    switch (elf.getNode(ni)) {
        ...
        inline .nav, .uav, .lazy_code, .lazy_const_data => |mi| {
            ...
            elf.flushMovedNodeRelocs(ni, new_addr, mi.firstSymbolReloc(elf), mi.firstGotReloc(elf));
        },
        .value_debug_info,
        .global_debug_info,
        .func_frame,
...
pub fn printNode(
    ...
    switch (node) {
        ...
        .global_debug_info => |gi| {
            const zcu = elf.base.comp.zcu.?;
            const ip = &zcu.intern_pool;
            const nav = ip.getNav(elf.dwarf.globals.keys()[@backingInt(gi)]);
            try w.print("({f}, {f})", .{
                Type.fromInterned(ip.typeOf(nav.resolved.?.value)).fmt(.{ .zcu = zcu, .tid = tid }),
                nav.fqn.fmt(ip),
            });
        },
        .func_frame, .func_debug_info, .func_debug_line => |fi| {
            const zcu = elf.base.comp.zcu.?;
            const ip = &zcu.intern_pool;
            const nav = ip.getNav(elf.dwarf.funcs.keys()[@backingInt(fi)]);
            try w.print("({f}, {f})", .{
                Type.fromInterned(ip.typeOf(nav.resolved.?.value)).fmt(.{ .zcu = zcu, .tid = tid }),
                nav.fqn.fmt(ip),
            });
